...
class PurchaseRequest(models.Model):
    ...
    def _get_default_name(self, company_id):
        ...
        if not name:
            raise UserError(
                    ...
                    % ('purchase.request', company_idd.name)
                )
        return name
    ...
    sale_id = fields.Many2one(comodel_name='sale.order', string='Sale Order', copy=False,)

    def init(self):
        company_ids = self.env['res.company'].search([])
        for comp in company_ids:
            seq_ids = self.env['ir.sequence'].search([('code', '=', 'purchase.request'), ('company_id', 'in', [comp.id, False])], order='company_id')
    ...
    def copy(self, default=None):
        ...
        self.ensure_one()
        default.update({"state": "draft", "name": self._get_default_name(self.company_id.id)})
    ...
    def create(self, vals):
        if vals.get("name", _("New")) == _("New"):
            vals["name"] = self._get_default_name(vals.get('company_id'))
