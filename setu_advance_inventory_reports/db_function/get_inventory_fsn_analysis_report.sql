...
BEGIN
    ...
    (
        Select
            t_data.*,
            case 
                when t_data.turnover_ratio > 0 and t_data.turnover_ratio >= 3 then 'Fast Moving'
                when t_data.turnover_ratio > 0 and t_data.turnover_ratio between 1 and 3 then 'Medium Moving'
                when t_data.turnover_ratio > 0 and t_data.turnover_ratio between 0 and 1 then 'Slow Moving'
                when t_data.turnover_ratio <= 0 then 'Non Moving' 
    ...
    --     else
    --         case when stock_movement_type = 'medium' then
    --             case when report_data.stock_movement = 'Medium Moving' then 1 else 0 end
    --         else
